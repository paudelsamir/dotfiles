My Hyprland setup based on end-4/dots and quickshell with some productive tweaks.

## Key Stuff:
- GPU monitoring in status bar
- Network speeds display  
- Pomodoro /timer /Stopwatch
- Todo system that persists
- Monthly goals tracker
- Better keybinds(clipboard, screenshots/records ,workspaces)
- Notifying widgets
- Minimal and productive Quickshell setup..

## Key Keybindings
- `Super + Q` - Close window
- `Super + E` - File manager (Dolphin)
- `Super + R` - VS Code
- `Super + V` - Toggle clipboard(gradia)
- `Super + Space` - App launcher
- `Super + Tab` - Overview/window switcher
- `Super + Shift + S` - Screenshot
- `Super + Shift + R` - Screen recording toggle
- `Super + tab` - Switch to next workspace
- `Super + Shift + tab` - Switch to previous workspace
- `Alt + [1-0]` - Move windows to workspace
- `Super + Alt + S` - Toggle window to/from scratchpad (hide/show windows)
- and many more.....

### Layouts
@LEFT SIDEBAR
- Todo System (daily tasks)
- Monthly Goals (long-term tracking)
- Tabbed interface with keyboard navigation
- Timer/Pomodoro controls
- Stopwatch functionality

@RIGHT SIDEBAR
- System notifications
- Calendar (standard + optional Nepali)
- Audio output controls

@QUICKSHELL
- Simply lovely

@Scratchpad 
- A super useful feature for managing windows - hide windows you want to keep running but don't need visible all the time!