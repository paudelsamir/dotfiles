...
// NOTE: This build of quickshell-git (0.3.0.r11) does not populate
// UPower.displayDevice / UPower.devices, so we read the battery straight from
// the UPower D-Bus service via busctl instead.
...
Singleton {
    ...
    // ─── Raw polled state (from UPower D-Bus) ───
    property bool _rawAvailable: false
    property real _rawPercentage: 1
    property int _rawState: 0 // UPowerDeviceState
    property real _rawEnergyRate: 0
    property real _rawTimeToEmpty: 0
    property real _rawTimeToFull: 0

    property bool available: _rawAvailable
    property var chargeState: _rawState
    property bool isCharging: chargeState == UPowerDeviceState.Charging
    property bool isPluggedIn: isCharging || chargeState == UPowerDeviceState.PendingCharge
    property real percentage: _rawPercentage
    ...
    property real energyRate: _rawEnergyRate
    property real timeToEmpty: _rawTimeToEmpty
    property real timeToFull: _rawTimeToFull

    // Material Symbols battery icon based on charge level + charging state.
    readonly property string materialSymbol: {
        if (!root.available) return "battery_unknown"
        const pct = root.percentage * 100
        const lvl = pct >= 95 ? "full"
            : pct >= 80 ? "6_bar"
            : pct >= 60 ? "5_bar"
            : pct >= 40 ? "4_bar"
            : pct >= 30 ? "3_bar"
            ...
            : pct >= 10 ? "1_bar"
            : "0_bar"
        if (root.isCharging) return `battery_charging_${lvl}`
        if (root.isCritical) return "battery_alert"
        return `battery_${lvl}`
    }

    function _readBattery(): void {
        if (batteryReader.running) return
        batteryReader.running = true
    }

    // Enumerate the first UPower battery device path and read its properties.
    Process {
        id: batteryReader
        running: true
        command: ["/bin/sh", "-c",
            "dev=$(busctl --system tree org.freedesktop.UPower 2>/dev/null | grep -oE '/org/freedesktop/UPower/devices/battery_[A-Za-z0-9_]+' | head -n1); " +
            "[ -n \"$dev\" ] || { echo 'NO_BATTERY'; exit 0; }; " +
            "PCT=$(busctl --system get-property org.freedesktop.UPower \"$dev\" org.freedesktop.UPower.Device Percentage 2>/dev/null | awk '{print $2}'); " +
            "STA=$(busctl --system get-property org.freedesktop.UPower \"$dev\" org.freedesktop.UPower.Device State 2>/dev/null | awk '{print $2}'); " +
            "RATE=$(busctl --system get-property org.freedesktop.UPower \"$dev\" org.freedesktop.UPower.Device EnergyRate 2>/dev/null | awk '{print $2}'); " +
            "TTE=$(busctl --system get-property org.freedesktop.UPower \"$dev\" org.freedesktop.UPower.Device TimeToEmpty 2>/dev/null | awk '{print $2}'); " +
            "TTF=$(busctl --system get-property org.freedesktop.UPower \"$dev\" org.freedesktop.UPower.Device TimeToFull 2>/dev/null | awk '{print $2}'); " +
            "printf 'OK %s %s %s %s %s\\n' \"${PCT:-0}\" \"${STA:-0}\" \"${RATE:-0}\" \"${TTE:-0}\" \"${TTF:-0}\""
        ]
        stdout: SplitParser {
            onRead: data => {
                const line = data.trim()
                if (line === "NO_BATTERY") {
                    root._rawAvailable = false
                    return
                }
                const parts = line.split(/\s+/)
                if (parts[0] !== "OK") return
                root._rawAvailable = true
                root._rawPercentage = parseFloat(parts[1]) / 100
                root._rawState = parseInt(parts[2])
                root._rawEnergyRate = parseFloat(parts[3]) || 0
                root._rawTimeToEmpty = parseFloat(parts[4]) || 0
                root._rawTimeToFull = parseFloat(parts[5]) || 0
                _log("[Battery] " + (root._rawPercentage * 100).toFixed(0) + "% state=" + root._rawState)
            }
        }
    }
