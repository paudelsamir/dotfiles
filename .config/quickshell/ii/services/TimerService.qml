...
Singleton {
    ...
    function refreshPomodoro() {
        ...
        if (getCurrentTimeInSeconds() >= Persistent.states.timer.pomodoro.start + pomodoroLapDuration) {
            ...
            // Send notification with enhanced audio
            ...
            if (Persistent.states.timer.pomodoro.isBreak && (pomodoroCycle + 1 == cyclesBeforeLongBreak)) {
                ...
                notificationMessage = `Take a long break: ${Math.floor(longBreakTime / 60)} minutes`;
            } else if (Persistent.states.timer.pomodoro.isBreak) {
                notificationTitle = "Break Time!";
                notificationMessage = `Take a break: ${Math.floor(breakTime / 60)} minutes`;
            } else {
                notificationTitle = "Focus Time!";
                notificationMessage = `Focus session: ${Math.floor(focusTime / 60)} minutes`;
            }

            // Enhanced notification with minimal style
            Quickshell.execDetached([
                ...
                "--category=pomodoro",
                "--hint=string:bgcolor:#1e1e2e",
                "--hint=string:fgcolor:#cdd6f4",
                "--hint=int:value:100",
                notificationTitle,
                notificationMessage
            ]);
            ...
            // Multiple sound fallbacks for better attention
            ...
            Quickshell.execDetached(["speaker-test", "-t", "sine", "-f", "800", "-l", "1"]);
    ...
    function stopwatchRecordLap() {
        ...
        // Minimal notification for lap recorded
        Quickshell.execDetached([
            "notify-send", 
            "--urgency=low",
            "--expire-time=3000",
            ...
            "--hint=string:fgcolor:#cdd6f4",
            "Lap Recorded",
            `Lap ${Persistent.states.timer.stopwatch.laps.length} saved`
        ]);
        
        // Subtle sound feedback
        Quickshell.execDetached(["paplay", "/usr/share/sounds/freedesktop/stereo/button-pressed.oga"]);
    }
}
