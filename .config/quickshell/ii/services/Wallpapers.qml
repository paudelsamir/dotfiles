...
Singleton {
    ...
    function openFallbackPicker(darkMode = Appearance.m3colors.darkmode) {
        applyProc.exec(["bash", "-c", `${Directories.wallpaperSwitchScriptPath} --mode ${darkMode ? "dark" : "light"}`])
    }

    function apply(path, darkMode = Appearance.m3colors.darkmode) {
        if (!path || path.length === 0) return
        applyProc.exec(["bash", "-c", `${Directories.wallpaperSwitchScriptPath} --image "${path}" --mode ${darkMode ? "dark" : "light"}`])
        root.changed()
    }
