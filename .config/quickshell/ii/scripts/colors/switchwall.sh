...
pre_process() {
    ...
    if [[ "$mode_flag" == "dark" ]]; then
        ...
        # Set Kvantum/Qt theme to dark
        sed -i 's/^theme=.*/theme=ColloidDark/' "$XDG_CONFIG_HOME/Kvantum/kvantum.kvconfig" 2>/dev/null
        sed -i 's/^style=.*/style=kvantum-dark/' "$XDG_CONFIG_HOME/qt5ct/qt5ct.conf" 2>/dev/null
        sed -i 's/^style=.*/style=kvantum-dark/' "$XDG_CONFIG_HOME/qt6ct/qt6ct.conf" 2>/dev/null
    elif [[ "$mode_flag" == "light" ]]; then
        gsettings set org.gnome.desktop.interface color-scheme 'prefer-light'
        gsettings set org.gnome.desktop.interface gtk-theme 'adw-gtk3'
        # Set Kvantum/Qt theme to light
        sed -i 's/^theme=.*/theme=Colloid/' "$XDG_CONFIG_HOME/Kvantum/kvantum.kvconfig" 2>/dev/null
        sed -i 's/^style=.*/style=kvantum/' "$XDG_CONFIG_HOME/qt5ct/qt5ct.conf" 2>/dev/null
        sed -i 's/^style=.*/style=kvantum/' "$XDG_CONFIG_HOME/qt6ct/qt6ct.conf" 2>/dev/null
    fi
...
switch() {
    ...
    if [[ -n "$mode_flag" ]]; then
        ...
        # Use the explicit mode_flag for materials. Only apply forceDarkMode if --mode was not explicitly passed.
        # (This check was already done earlier; if we reach here, the mode was explicitly set by user)
