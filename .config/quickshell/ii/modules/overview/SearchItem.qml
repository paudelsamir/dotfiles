...
RippleButton {
    ...
    // Cache highlighting to avoid recalculation
    property string _cachedQuery: ""
    property string _cachedName: ""
    property string _cachedHighlight: ""
    
    function highlightContent(content, query) {
        // Use cached result if inputs haven't changed
        if (_cachedQuery === query && _cachedName === content) {
            return _cachedHighlight;
        }
        
        if (!query || query.length === 0 || content == query || fontType === "monospace") {
            _cachedHighlight = StringUtils.escapeHtml(content);
            _cachedQuery = query;
            _cachedName = content;
            return _cachedHighlight;
        }

        // Simplified highlighting - only highlight if query is short (< 20 chars) to avoid lag
        if (query.length > 20) {
            _cachedHighlight = StringUtils.escapeHtml(content);
            _cachedQuery = query;
            _cachedName = content;
        ...
        // Simple substring highlighting instead of fuzzy matching
        let startIndex = contentLower.indexOf(queryLower);
        if (startIndex >= 0) {
            let beforeMatch = StringUtils.escapeHtml(content.slice(0, startIndex));
            let match = StringUtils.escapeHtml(content.slice(startIndex, startIndex + query.length));
            let afterMatch = StringUtils.escapeHtml(content.slice(startIndex + query.length));
            _cachedHighlight = beforeMatch + root.highlightPrefix + match + root.highlightSuffix + afterMatch;
        } else {
            _cachedHighlight = StringUtils.escapeHtml(content);
        }
    ...
    // Cache URL detection to avoid regex on every update
    property string _cachedUrlName: ""
    property list<string> _cachedUrls: []
    
    property list<string> urls: {
        if (!root.itemName) return [];
        
        // Use cached result if name hasn't changed
        if (_cachedUrlName === root.itemName) {
            return _cachedUrls;
        }
        
        // Only run URL detection for clipboard entries (most likely to have URLs)
        if (!root.cliphistRawString) {
            _cachedUrls = [];
            _cachedUrlName = root.itemName;
            return [];
        }
        
        // Regular expression to match URLs - only for clipboard entries
        const urlRegex = /https?:\/\/[^\s<>"{}|\\^`[\]]+/gi;
        const matches = root.itemName?.match(urlRegex)
            ?.filter(url => !url.includes("…")) // Elided = invalid
        
        _cachedUrls = matches ? matches : [];
        _cachedUrlName = root.itemName;
        return _cachedUrls;
    }
