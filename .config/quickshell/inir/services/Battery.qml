...
Singleton {
    ...
    readonly property string materialSymbol: {
        if (isCharging || percentage >= 1) return "battery_charging_full"
        if (percentage < 0.15) return "battery_0_bar"
        if (percentage < 0.35) return "battery_2_bar"
        if (percentage < 0.55) return "battery_3_bar"
        if (percentage < 0.75) return "battery_4_bar"
        return "battery_full"
    }
