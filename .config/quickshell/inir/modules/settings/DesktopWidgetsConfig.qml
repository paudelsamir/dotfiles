...
ContentPage {
    ...
    // Keys driven by WidgetAppearanceControls; exclude them from the
    // manifest-driven auto controls so custom widgets don't show duplicates.
    readonly property var _appearanceManagedKeys: ({
        showBackground: true, useBlur: true, showBorder: true,
        backgroundOpacity: true, borderWidth: true, borderOpacity: true,
        cornerRadius: true, colorMode: true, widgetScale: true,
        widgetOpacity: true, dim: true, placementStrategy: true,
        locked: true, x: true, y: true
    })

    function _customContentConfigKeys(configKeys: var): var {
        const keys = configKeys ?? {};
        const standard = root._appearanceManagedKeys;
        return Object.keys(keys).filter(k => !standard[k]);
    }
    ...
    SettingsCardSection {
        ...
        Repeater {
            ...
            SettingsGroup {
                ...
                ContentSubsection {
                    visible: root._customContentConfigKeys(cwDelegate.modelData.configKeys).length > 0
                    title: Translation.tr("Settings")

                    Repeater {
                        model: {
                            const keys = cwDelegate.modelData.configKeys || {};
                            return root._customContentConfigKeys(keys).map(k => ({
                                key: k, spec: keys[k],
                ...
                WidgetAppearanceControls {
                    ...
                    hasSemanticPalette: Boolean(cwDelegate.modelData.semanticPalette)
                }
            }
        }
    }
