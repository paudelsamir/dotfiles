        ...
        sourceComponent: PanelWindow {
            ...
            FocusScope {
                ...
                readonly property real targetY: {
                    if (popupMode === "dock")
                        return screenH - height - 20
                    if (Config.options?.bar?.bottom ?? false)
                        return screenH - height - Appearance.sizes.baseBarHeight - 5
                    return Appearance.sizes.baseBarHeight + 5
                }

                y: cardArea.targetY
                opacity: 0
                scale: 0.9
                transformOrigin: popupMode === "dock" ? Item.Bottom : (Config.options?.bar?.bottom ?? false) ? Item.Bottom : Item.Top
